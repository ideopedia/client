...
function Signup() {
  ...
  const [invite,dispatchInvite] = useReducer(inviteReducer,{value:"",isValid:true})
  ...
  const loginSubmitHandler = (e) => {
    e.preventDefault();
    if (formValid) {
      const user = { email: email.value, password: password.value,inviteCode:invite.value };
      console.log(user);
    }
    if (!emailValid) {
  ...
  const inviteClass=`${inviteValid?" ":"focus:ring-red-500 focus:border-red-500"}`
  return (
    <div>
      <Navbar />
      ...
      <div className="newLoginTop" style={{ marginLeft: "-1.8rem" }}>
        <FormHeader title="Email" />
          ...
          <input
            ...
            required={true}
          />
          {!inviteValid&&<p style={{fontSize:"12px",position:"relative",bottom:"10px",left:"10px",color:"red"}}>Enter a valid invite code</p>}
